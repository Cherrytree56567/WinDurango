...
__int64 __fastcall GetCurrentPackageFullName_X(std::wstring &packageFullName)
{
    ...
    LSTATUS status = RegOpenKeyExW(HKEY_LOCAL_MACHINE,
                                   L"XBOX\Software\Microsoft\Windows\CurrentVersion\AppModel\Repository\Families", 0,
                                   KEY_READ, &hKey);
...
__int64 __fastcall GetPackagesByPackageFamily_X(LPCWSTR lpSubKey, std::vector<std::wstring> &packageList)
{
    ...
    LSTATUS status = RegOpenKeyExW(
        HKEY_LOCAL_MACHINE, L"XBOX\\Software\\Microsoft\\Windows\\CurrentVersion\\AppModel\\Repository\\Families", 0,
        KEY_READ, &hKey);
...
HRESULT CreateRandomAccessStreamOnFile_X(PCWSTR filePath, DWORD accessMode, REFIID riid, void **ppv)
{
    return TRUE;
}
...
LONG __stdcall GetApplicationUserModelId_X(HANDLE hProcess, UINT32 *applicationUserModelIdLength,
...
LONG GetCurrentPackageInfo_X(const UINT32 flags, UINT32 *bufferLength, BYTE *buffer, UINT32 *count)
{
    return 0;
}

LONG __stdcall GetPackagesByPackageFamily_X(PCWSTR packageFamilyName, UINT32 *count, PWSTR *packageFullNames,
                                            UINT32 *bufferLength, WCHAR *buffer)
{
    ...
    {
        wcsncpy_s(currentBufferPosition, *bufferLength - (currentBufferPosition - buffer), packageList[i].c_str(),
                  packageList[i].size());
...
LSTATUS __fastcall XblRegReadValue_X(HKEY hKey, LPCWSTR valueName, int expectedType, void *buffer,
...
__int64 __fastcall XblRegReadModuleTitleInfo_X(HINSTANCE a1, const unsigned __int16 *a2, void *a3, unsigned int a4)
{
    ...
    if (result == ERROR_SUCCESS)
    {
        if (XblRegReadValue_X(hKey, L"UseSystemTitleId", REG_DWORD, &useSystemTitleId, sizeof(useSystemTitleId)) !=
                ERROR_SUCCESS ||
            !useSystemTitleId)
        {
...
__int64 __fastcall XblRegOpenModuleKey_X(HMODULE a1, __int64 a2, HKEY *a3)
{
    ...
    if (a1 && GetModuleFileNameW(a1, Filename, 0x104))
    {
        ...
        if (SUCCEEDED(StringCchPrintfW(SubKey, 0x200,
                                       L"XBOX\\Software\\Microsoft\\Windows\\CurrentVersion\\XboxLive\\TitleIds\\%s",
                                       Filename)))
