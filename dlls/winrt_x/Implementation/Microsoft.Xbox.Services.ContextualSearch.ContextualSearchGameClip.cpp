#include "Microsoft.Xbox.Services.ContextualSearch.ContextualSearchGameClip.h"
#include "Microsoft.Xbox.Services.ContextualSearch.ContextualSearchGameClip.g.cpp"
#include "pch.h"

namespace winrt::Microsoft::Xbox::Services::ContextualSearch::implementation
{
hstring ContextualSearchGameClip::ClipName()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
uint64_t ContextualSearchGameClip::DurationInSeconds()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
hstring ContextualSearchGameClip::GameClipId()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
hstring ContextualSearchGameClip::GameClipLocale()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
winrt::Windows::Foundation::Collections::IVectorView<
    winrt::Microsoft::Xbox::Services::ContextualSearch::ContextualSearchGameClipUriInfo>
ContextualSearchGameClip::GameClipUris()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
winrt::Windows::Foundation::Collections::IVectorView<
    winrt::Microsoft::Xbox::Services::ContextualSearch::ContextualSearchGameClipThumbnail>
ContextualSearchGameClip::Thumbnails()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
winrt::Microsoft::Xbox::Services::ContextualSearch::ContextualSearchGameClipType ContextualSearchGameClip::
    GameClipType()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
uint64_t ContextualSearchGameClip::Views()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
hstring ContextualSearchGameClip::XboxUserId()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
winrt::Windows::Foundation::Collections::IVectorView<
    winrt::Microsoft::Xbox::Services::ContextualSearch::ContextualSearchGameClipStat>
ContextualSearchGameClip::Stats()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
} // namespace winrt::Microsoft::Xbox::Services::ContextualSearch::implementation
