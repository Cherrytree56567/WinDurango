#include "Microsoft.Xbox.Services.Marketplace.CatalogItemDetails.h"
#include "Microsoft.Xbox.Services.Marketplace.CatalogItemDetails.g.cpp"
#include "pch.h"

namespace winrt::Microsoft::Xbox::Services::Marketplace::implementation
{
winrt::Microsoft::Xbox::Services::Marketplace::MediaItemType CatalogItemDetails::MediaItemType()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
hstring CatalogItemDetails::Id()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
hstring CatalogItemDetails::Name()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
hstring CatalogItemDetails::ReducedName()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
hstring CatalogItemDetails::Description()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
uint32_t CatalogItemDetails::TitleId()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
winrt::Windows::Foundation::DateTime CatalogItemDetails::ReleaseDate()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
hstring CatalogItemDetails::ProductId()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
hstring CatalogItemDetails::SandboxId()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
bool CatalogItemDetails::IsBundle()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
bool CatalogItemDetails::IsPartOfAnyBundle()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
winrt::Windows::Foundation::Collections::IVectorView<winrt::Microsoft::Xbox::Services::Marketplace::CatalogItemImage>
CatalogItemDetails::Images()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
winrt::Windows::Foundation::Collections::IVectorView<
    winrt::Microsoft::Xbox::Services::Marketplace::CatalogItemAvailability>
CatalogItemDetails::Availabilities()
{
    LOG_NOT_IMPLEMENTED();
    throw hresult_not_implemented();
}
} // namespace winrt::Microsoft::Xbox::Services::Marketplace::implementation
