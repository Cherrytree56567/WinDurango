...
class CoreApplicationWrapperX : public RuntimeClass<IActivationFactory, ICoreApplicationResourceAvailabilityX,
                                                    ICoreApplicationGpuPolicy, ICoreApplicationX, ICoreApplicationExit>
{
  public:
    CoreApplicationWrapperX(ComPtr<IActivationFactory> realFactory) : m_realFactory(realFactory)
    {
    ...
    HRESULT STDMETHODCALLTYPE add_Exiting(__FIEventHandler_1_IInspectable *handler,
                                          EventRegistrationToken *token) override;

    HRESULT STDMETHODCALLTYPE remove_Exiting(EventRegistrationToken token) override;
