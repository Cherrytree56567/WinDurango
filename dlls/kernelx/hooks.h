...
#define RETURN_LAST_ERROR_IF(cond)                                                                                     \
    if (cond)                                                                                                          \
...
HRESULT(STDMETHODCALLTYPE *TrueGetLicenseInformation)(
    ABI::Windows::ApplicationModel::Store::ILicenseInformation **value) = nullptr;
...
HRESULT __stdcall CoCreateInstance_hook(_In_ REFCLSID rclsid, _In_opt_ LPUNKNOWN pUnkOuter, _In_ DWORD dwClsContext,
                                        _In_ REFIID riid,
                                        _COM_Outptr_ _At_(*ppv, _Post_readable_size_(_Inexpressible_(varies)))
                                            LPVOID FAR *ppv)
{
...
/// Made By XWine1 Team
HRESULT XGetImport(_In_opt_ HMODULE Module, _In_ HMODULE ImportModule, _In_ LPCSTR Import,
...
HMODULE WINAPI LoadLibraryExW_X(LPCWSTR lpLibFileName, HANDLE hFile, DWORD dwFlags)
{
    ...
    if (wcscmp(lpLibFileName, L"xaudio2_9.dll") == 0 || wcscmp(lpLibFileName, L"xaudio2_9d.dll") == 0)
    {
        ...
        if (wcscmp(currentPath, callerPath) == 0 && !(wcscmp(callerfileName, L"xaudio2_9_x.dll") == 0))
        {
    ...
    PatchNeededImports(mod, GetRuntimeModule(), "?GetActivationFactoryByPCWSTR@@YAJPEAXAEAVGuid@Platform@@PEAPEAX@Z",
...
HANDLE CreateFile2_Hook(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, DWORD dwCreationDisposition,
...
HANDLE WINAPI CreateFileW_Hook(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
                               LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition,
...
HRESULT STDMETHODCALLTYPE CurrentAppActivateInstance_Hook(IActivationFactory *thisptr, IInspectable **instance)
{
    ...
    *instance = reinterpret_cast<Store::ILicenseInformation *>(
        new LicenseInformationWrapperX(reinterpret_cast<Store::ILicenseInformation *>(*instance)));
    return hr;
}
...
inline HRESULT WINAPI RoGetActivationFactory_Hook(HSTRING classId, REFIID iid, void **factory)
{
    ...
    if (IsClassName(classId, "Windows.ApplicationModel.Store.CurrentApp"))
    {
        ...
        // @unixian: is there a better way to do this? it works, but we never know if the vtable will change (microsoft
        // please don't make breaking ABI changes)
    ...
    if (IsClassName(classId, "Windows.UI.Core.CoreWindow"))
    {
        ...
        hr = TrueRoGetActivationFactory(HStringReference(RuntimeClass_Windows_UI_Core_CoreWindow).Get(),
    ...
    if (!pDllGetActivationFactory)
    {
        ...
        pDllGetActivationFactory =
            reinterpret_cast<DllGetActivationFactoryFunc>(GetProcAddress(library, "DllGetActivationFactory"));
