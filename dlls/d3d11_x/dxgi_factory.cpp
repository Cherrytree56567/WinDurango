...
#define DXGI_SWAPCHAIN_FLAG_MASK                                                                                       \
    DXGI_SWAP_CHAIN_FLAG_NONPREROTATED | DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH |                                      \
        DXGI_SWAP_CHAIN_FLAG_GDI_COMPATIBLE | DXGI_SWAP_CHAIN_FLAG_RESTRICTED_CONTENT |                                \
        DXGI_SWAP_CHAIN_FLAG_RESTRICT_SHARED_RESOURCE_DRIVER | DXGI_SWAP_CHAIN_FLAG_DISPLAY_ONLY |                     \
        DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT | DXGI_SWAP_CHAIN_FLAG_FOREGROUND_LAYER |                   \
        DXGI_SWAP_CHAIN_FLAG_FULLSCREEN_VIDEO | DXGI_SWAP_CHAIN_FLAG_YUV_VIDEO | DXGI_SWAP_CHAIN_FLAG_HW_PROTECTED |   \
        DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING | DXGI_SWAP_CHAIN_FLAG_RESTRICTED_TO_ALL_HOLOGRAPHIC_DISPLAYS
...
{
    return wrapped_interface->CreateSwapChainForComposition(reinterpret_cast<IUnknown *>(pDevice), pDesc,
...
HRESULT wd::dxgi_factory::CreateSwapChainForHwnd(IGraphicsUnknown *pDevice, HWND hWnd,
                                                 ...
                                                 IDXGIOutput *pRestrictToOutput, IDXGISwapChain1 **ppSwapChain)
{
    return wrapped_interface->CreateSwapChainForHwnd(reinterpret_cast<IUnknown *>(pDevice), hWnd, pDesc,
                                                       ...
                                                       IDXGISwapChain1 **ppSwapChain)
{
    ...
    if (pWindow == nullptr)
    {
        ...
        hr = RoGetActivationFactory(
            Microsoft::WRL::Wrappers::HStringReference(RuntimeClass_Windows_UI_Core_CoreWindow).Get(),
    ...
    else
    {
        ...
        hr = wrapped_interface->CreateSwapChainForCoreWindow(pRealDevice, windowWrapper->m_realWindow, pDesc,
                                                             pRestrictToOutput, &swap);
        if (FAILED(hr))
        {
