...
wd::Overlay::Overlay(ID3D11Device *pDevice, ID3D11DeviceContext *pContext, IDXGISwapChain1 *pSwapchain)
    : m_pDevice(pDevice), m_pContext(pContext), m_pSwapchain(pSwapchain), m_pRenderTargetView(nullptr),
      m_bKeyboard(false)
{
}
...
void SetupImGuiKeyboardInputUWP()
{
    ...
    window.KeyDown(TypedEventHandler<CoreWindow, KeyEventArgs>([](CoreWindow const &, KeyEventArgs const &args) {
        ImGuiKey key = MapVirtualKeyToImGuiKey(args.VirtualKey());
        if (key != ImGuiKey_None)
            ImGui::GetIO().AddKeyEvent(key, true);
    }));

    window.KeyUp(TypedEventHandler<CoreWindow, KeyEventArgs>([](CoreWindow const &, KeyEventArgs const &args) {
        ImGuiKey key = MapVirtualKeyToImGuiKey(args.VirtualKey());
        if (key != ImGuiKey_None)
...
void wd::Overlay::RenderKeyboardWindow()
{
    ...
    const char *keys[] = {"1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "Q", "W", "E", "R",
    ...
    ImGui::GetWindowDrawList()->AddText(
        ...
        IM_COL32(255, 255, 255, 255), "\xEF\x84\x8A");
    ...
    ImGui::SameLine();
    if (ImGui::Button("##del", ImVec2(60, 30)) || ImGui::IsKeyPressed(ImGuiKey_Backspace) ||
    ...
    ImGui::GetWindowDrawList()->AddText(
        ...
        IM_COL32(255, 255, 255, 255), "\xEE\x9D\x90");
    ...
    ImGui::GetWindowDrawList()->AddText(
        ...
        IM_COL32(0, 149, 209, 255), "\xEF\x82\x96");
    ImGui::PopStyleVar();
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(3, 3));

    if (ImGui::Button("##bac", ImVec2(60.0f, 63)) || ImGui::IsKeyPressed(ImGuiKey_LeftShift) ||
    ...
    ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), 18,
                                        ImVec2((pos.x + (size.x - bigTextSize.x) * 0.5f) + 6, pos.y + 18),
                                        IM_COL32(255, 255, 255, 255), "\xEE\x80\x8F");

    smallTextSize = ImGui::CalcTextSize("\xEF\x84\x8D");
    ImGui::GetWindowDrawList()->AddText(
        ImGui::GetFont(), 14, ImVec2((pos.x + (size.x - smallTextSize.x) * 0.5f) + 8, pos.y + 20 + bigTextSize.y),
        IM_COL32(255, 255, 255, 255), "\xEF\x84\x8D");
    ...
    if (!isSymbols)
        RenderKeyboardRow(currentKeys, 20, 30, isUppercase);
    else
        RenderKeyboardRow(currentKeys, 20, 30, isUppercase);
    ...
    if (!isUppercase)
    {
        ...
        ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), 18,
                                            ImVec2((pos.x + (size.x - bigTextSize.x) * 0.5f) - 3, pos.y + 18),
                                            IM_COL32(255, 255, 255, 255), "\xEE\x9D\x92");
    }
    else
    {
        ...
        ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), 18,
                                            ImVec2((pos.x + (size.x - bigTextSize.x) * 0.5f) - 3, pos.y + 18),
                                            IM_COL32(255, 255, 255, 255), "\xEE\xA2\x96");
    }

    smallTextSize = ImGui::CalcTextSize("\xEF\x84\x88");
    ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), 14,
                                        ImVec2(pos.x + (size.x - smallTextSize.x) * 0.5f, pos.y + 26 + bigTextSize.y),
                                        IM_COL32(255, 255, 255, 255), "\xEF\x84\x88");

    ImGui::SameLine();
    currPos = ImGui::GetCursorPos();
    if (!isSymbols)
        RenderKeyboardRow(currentKeys, 30, 40, isUppercase);

    ImGui::PopStyleVar();
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0, 3));
    ImGui::SameLine();
    if (ImGui::Button("##enter", ImVec2(60.0f, 63)) || ImGui::IsKeyPressed(ImGuiKey_Enter) ||
    ...
    ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), 18,
                                        ...
                                        IM_COL32(255, 255, 255, 255), "\xEE\xAF\xA7");

    smallTextSize = ImGui::CalcTextSize("\xEE\xB7\xA3");
    ImGui::GetWindowDrawList()->AddText(
        ImGui::GetFont(), 12, ImVec2((pos.x + (size.x - smallTextSize.x) * 0.5f) + 5, pos.y + 26 + bigTextSize.y),
        IM_COL32(255, 255, 255, 255), "\xEE\xB7\xA3");
    ImGui::PopStyleVar();
...
void wd::Overlay::HandleKeyboardSpecialKeys()
{
    ...
    if (ImGui::Button("##space", ImVec2((ImGui::GetContentRegionAvail().x - 63), 30)) ||
