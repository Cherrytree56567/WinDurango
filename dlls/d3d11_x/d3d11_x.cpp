...
HRESULT CreateDevice(UINT Flags, wdi::ID3D11Device **ppDevice, wdi::ID3D11DeviceContext **ppImmediateContext)
{
    ...
    HRESULT hr =
        D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, flags, featurelevels, _ARRAYSIZE(featurelevels),
                          D3D11_SDK_VERSION, reinterpret_cast<ID3D11Device **>(ppDevice), NULL,
                          reinterpret_cast<ID3D11DeviceContext **>(ppImmediateContext));
    if (SUCCEEDED(hr))
    {
        ...
        if (ppImmediateContext != nullptr)
        {
            ...
            *ppImmediateContext =
                reinterpret_cast<wdi::ID3D11DeviceContext *>(new wd::device_context_x(device_context2));
        }
    }
    else
    {
...
HRESULT __stdcall D3DMapEsramMemory_X(UINT Flags, VOID *pVirtualAddress, UINT NumPages, const UINT *pPageList)
{
    ...
    hDevice = CreateFileW(L"\\\\.\\VdKmd", GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING,
...
HRESULT __stdcall VdMapAddressToEsram(HANDLE hDevice, DWORD flags, uintptr_t virtualAddress, UINT numPages,
                                      const UINT *pageList)
{
    ...
    if (((flags & 0x20000000) && (virtualAddress == 0)) ||
        (!(flags & 0x20000000) && (virtualAddress & 0x3FFFFF) != 0) || (numPages > pageLimit + 1))
    {
...
HRESULT _stdcall D3DUploadCustomMicrocode_X(_In_ UINT32 MicrocodeVersion, _In_ SIZE_T CeMicrocodeSizeBytes,
...
HRESULT __stdcall D3D11XCreateDeviceXAndSwapChain1_X(
    const D3D11X_CREATE_DEVICE_PARAMETERS *pParameters, const DXGI_SWAP_CHAIN_DESC1 *pSwapChainDesc,
    ...
    ID3D11Device **ppDevice, ID3D11DeviceContext **ppImmediateContext)
{
    ...
    return D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE, 0, pParameters->Flags & CREATE_DEVICE_FLAG_MASK, NULL,
                             NULL, D3D11_SDK_VERSION, ppDevice, NULL, ppImmediateContext);
}

HRESULT __stdcall D3DAllocateGraphicsMemory_X(SIZE_T SizeBytes, SIZE_T AlignmentBytes, UINT64 DesiredGpuVirtualAddress,
                                              UINT Flags, void **ppAddress)
{
    DWORD Protect = 0;
...
HRESULT __stdcall DXGIXGetFrameStatistics_X(_In_ UINT NumberFramesRequested,
                                            _Out_writes_(NumberFramesRequested) DXGIX_FRAME_STATISTICS *pStats)
{
    return E_NOTIMPL;
}

HRESULT _stdcall DXGIXPresentArray_X(_In_ UINT SyncInterval, _In_ UINT PresentImmediateThreshold, _In_ UINT Flags,
                                     _In_ UINT NumSwapChains, _In_ IDXGISwapChain1 *const *ppSwapChain,
...
HRESULT __stdcall DXGIXSetFrameNotification_X(_In_ DXGIX_FRAME_NOTIFICATION NotificationType, _In_ HANDLE hEvent)
{
    return E_NOTIMPL;
}

HRESULT __stdcall DXGIXSetVLineNotification_X(_In_ DXGIX_VLINECOUNTER VLineCounter, _In_ UINT32 VLineNumber,
                                              _In_opt_ HANDLE hEvent)
{
    return E_NOTIMPL;
}

HRESULT __stdcall D3D11CreateDevice_X(_In_opt_ IDXGIAdapter *pAdapter, D3D_DRIVER_TYPE DriverType, HMODULE Software,
                                      UINT Flags, _In_reads_opt_(FeatureLevels) CONST D3D_FEATURE_LEVEL *pFeatureLevels,
                                      UINT FeatureLevels, UINT SDKVersion, _Out_opt_ wdi::ID3D11Device **ppDevice,
                                      ...
                                      _Out_opt_ wdi::ID3D11DeviceContext **ppImmediateContext)
{
    ...
    HRESULT hr =
        D3D11CreateDevice(pAdapter, DriverType, Software, flags, featurelevels, _ARRAYSIZE(featurelevels), SDKVersion,
                          (ID3D11Device **)ppDevice, pFeatureLevel, (ID3D11DeviceContext **)ppImmediateContext);
...
HRESULT __stdcall D3D11XCreateDeviceX_X(_In_ const D3D11X_CREATE_DEVICE_PARAMETERS *pParameters,
...
HRESULT __stdcall D3D11CreateDeviceAndSwapChain_X(
    _In_opt_ IDXGIAdapter *pAdapter, D3D_DRIVER_TYPE DriverType, HMODULE Software, UINT Flags,
