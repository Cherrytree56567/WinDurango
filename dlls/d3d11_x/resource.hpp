...
// @unixian: does this even need to be wrapped? seems like the only thing it does in the pre-rewrite code is handle
// GetDevice
namespace wd
{
// this is only used for casting to other resources, as it's impossible to make a resource itself (has to be a texture
// or buffer)
...
{
  public:
    texture_1d(::ID3D11Texture1D *texture) : wrapped_interface(texture)
    {
...
{
  public:
    texture_2d(::ID3D11Texture2D *texture) : wrapped_interface(texture)
    {
        wrapped_interface->AddRef();
    }
...
{
  public:
    texture_3d(::ID3D11Texture3D *texture) : wrapped_interface(texture)
    {
        wrapped_interface->AddRef();
    }
...
{
  public:
    buffer(::ID3D11Buffer *buffer) : wrapped_interface(buffer)
    {
        wrapped_interface->AddRef();
    }
