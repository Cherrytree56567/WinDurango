...
typedef enum D3D11X_GPU_PIPELINED_EVENT
{
    ...
    D3D11X_GPU_PIPELINED_EVENT_CS_PARTIAL_FLUSH =
        0x00000407, // Ensure all CS finished running before dispatching new CS
    D3D11X_GPU_PIPELINED_EVENT_VS_PARTIAL_FLUSH =
        0x0000040f, // Ensure all VS finished running before dispatching new VS
    D3D11X_GPU_PIPELINED_EVENT_PS_PARTIAL_FLUSH =
        0x00000410,                                      // Ensure all PS finished running before dispatching new PS
...
enum D3D11X_SHADER_CS_SIMD_WALK_ALGORITHM : UINT
{
    ...
    D3D11X_SHADER_CS_SIMD_WALK_ALGORITHM_GLOBAL_BALANCED =
        1, // Walk the SIMDs across CUs but maintaining a balanced distribution per CU.
    D3D11X_SHADER_CS_SIMD_WALK_ALGORITHM_GLOBAL_STRICT =
        2, // Walk the SIMDs across CUs disregarding wave distribution within each CU.
...
typedef enum D3D11X_THREAD_TRACE_PIPELINE_ENABLE
{
    ...
    D3D11X_THREAD_TRACE_PIPELINE_ENABLE_ALL =
        0x00000001, //  Trace target-CU details for all threads of this type when tracing is enabled
...
D3DINTERFACE(ID3D11DeviceContext, c0bfa96c, e089, 44fb, 8e, af, 26, f8, 79, 61, 90, da) : public ID3D11DeviceChild
{
  public:
    ...
    virtual void(PSSetShaderResources)(ID3D11ShaderResourceView *const *ppShaderResourceViews,
                                       _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT StartSlot,
                                       _In_ UINT PacketHeader) = 0;
    virtual void(PSSetShader)(_In_opt_ ID3D11PixelShader * pPixelShader) = 0;
    virtual void(PSSetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
    ...
    virtual HRESULT(Map)(_In_ ID3D11Resource * pResource, _In_ UINT Subresource, _In_ D3D11_MAP MapType,
                         _In_ UINT MapFlags, _Out_ D3D11_MAPPED_SUBRESOURCE * pMappedResource) = 0;
    ...
    virtual void(IASetVertexBuffers)(
        ...
        _In_reads_opt_(NumBuffers) const UINT *pStrides, _In_reads_opt_(NumBuffers) const UINT *pOffsets) = 0;
    virtual void(IASetIndexBuffer)(_In_ UINT HardwareIndexFormat, _In_opt_ ID3D11Buffer * pIndexBuffer,
                                   _In_ UINT Offset) = 0;
    virtual void(DrawIndexedInstanced)(_In_ UINT64 StartIndexLocationAndIndexCountPerInstance,
                                       _In_ UINT64 BaseVertexLocationAndStartInstanceLocation,
                                       _In_ UINT InstanceCount) = 0;
    virtual void(DrawInstanced)(_In_ UINT VertexCountPerInstance,
                                _In_ UINT64 StartVertexLocationAndStartInstanceLocation, _In_ UINT InstanceCount) = 0;
    virtual void(GSSetConstantBuffers)(
        _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1) UINT StartSlot,
        _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot) UINT NumBuffers,
        _In_reads_opt_(NumBuffers) ID3D11Buffer *const *ppConstantBuffers) = 0;
    virtual void(GSSetShader)(_In_opt_ ID3D11GeometryShader * pShader) = 0;
    virtual void(VSSetShaderResources)(ID3D11ShaderResourceView *const *ppShaderResourceViews,
                                       _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT StartSlot,
                                       _In_ UINT PacketHeader) = 0;
    virtual void(VSSetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _In_reads_opt_(NumSamplers) ID3D11SamplerState *const *ppSamplers) = 0;
    virtual void(Begin)(_In_ ID3D11Asynchronous * pAsync) = 0;
    virtual void(End)(_In_ ID3D11Asynchronous * pAsync) = 0;
    virtual HRESULT(GetData)(_In_ ID3D11Asynchronous * pAsync, _Out_writes_bytes_opt_(DataSize) void *pData,
                             _In_ UINT DataSize, _In_ UINT GetDataFlags) = 0;
    virtual void(SetPredication)(_In_opt_ ID3D11Predicate * pPredicate, _In_ BOOL PredicateValue) = 0;
    virtual void(GSSetShaderResources)(ID3D11ShaderResourceView *const *ppShaderResourceViews,
                                       _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT StartSlot,
                                       _In_ UINT PacketHeader) = 0;
    virtual void(GSSetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _In_reads_opt_(NumSamplers) ID3D11SamplerState *const *ppSamplers) = 0;
    virtual void(OMSetRenderTargets)(_In_range_(0, D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT) UINT NumViews,
                                     _In_reads_opt_(NumViews) ID3D11RenderTargetView *const *ppRenderTargetViews,
                                     _In_opt_ ID3D11DepthStencilView *pDepthStencilView) = 0;
    virtual void(OMSetRenderTargetsAndUnorderedAccessViews)(
        _In_ UINT NumRTVs, _In_reads_opt_(NumRTVs) ID3D11RenderTargetView *const *ppRenderTargetViews,
        _In_opt_ ID3D11DepthStencilView *pDepthStencilView, _In_range_(0, D3D11_1_UAV_SLOT_COUNT - 1) UINT UAVStartSlot,
        _In_ UINT NumUAVs, _In_reads_opt_(NumUAVs) ID3D11UnorderedAccessView *const *ppUnorderedAccessViews,
        _In_reads_opt_(NumUAVs) const UINT *pUAVInitialCounts) = 0;
    virtual void(OMSetBlendState)(_In_opt_ ID3D11BlendState * pBlendState, _In_opt_ const FLOAT BlendFactor[4],
                                  _In_ UINT SampleMask) = 0;
    virtual void(OMSetDepthStencilState)(_In_opt_ ID3D11DepthStencilState * pDepthStencilState,
                                         _In_ UINT StencilRef) = 0;
    virtual void(SOSetTargets)(_In_range_(0, D3D11_SO_BUFFER_SLOT_COUNT) UINT NumBuffers,
                               _In_reads_opt_(NumBuffers) ID3D11Buffer *const *ppSOTargets,
                               _In_reads_opt_(NumBuffers) const UINT *pOffsets) = 0;
    virtual void(DrawAuto)() = 0;
    virtual void(DrawIndexedInstancedIndirect)(_In_ ID3D11Buffer * pBufferForArgs,
    ...
    virtual void(RSSetViewports)(_In_range_(0, D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE)
                                     UINT NumViewports,
                                 _In_reads_opt_(NumViewports) const D3D11_VIEWPORT *pViewports) = 0;
    virtual void(RSSetScissorRects)(_In_range_(0, D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE)
                                        UINT NumRects,
                                    _In_reads_opt_(NumRects) const D3D11_RECT *pRects) = 0;
    virtual void(CopySubresourceRegion)(_In_ ID3D11Resource * pDstResource, _In_ UINT DstSubresource, _In_ UINT DstX,
                                        _In_ UINT DstY, _In_ UINT DstZ, _In_ ID3D11Resource * pSrcResource,
                                        _In_ UINT SrcSubresource, _In_opt_ const D3D11_BOX *pSrcBox) = 0;
    virtual void(CopyResource)(_In_ ID3D11Resource * pDstResource, _In_ ID3D11Resource * pSrcResource) = 0;
    virtual void(UpdateSubresource)(_In_ ID3D11Resource * pDstResource, _In_ UINT DstSubresource,
                                    _In_opt_ const D3D11_BOX *pDstBox, _In_ const void *pSrcData, _In_ UINT SrcRowPitch,
                                    _In_ UINT SrcDepthPitch) = 0;
    virtual void(CopyStructureCount)(_In_ ID3D11Buffer * pDstBuffer, _In_ UINT DstAlignedByteOffset,
                                     _In_ ID3D11UnorderedAccessView * pSrcView) = 0;
    virtual void(ClearRenderTargetView)(_In_ ID3D11RenderTargetView * pRenderTargetView,
                                        _In_ const FLOAT ColorRGBA[4]) = 0;
    virtual void(ClearUnorderedAccessViewUint)(_In_ ID3D11UnorderedAccessView * pUnorderedAccessView,
                                               _In_ const UINT Values[4]) = 0;
    virtual void(ClearUnorderedAccessViewFloat)(_In_ ID3D11UnorderedAccessView * pUnorderedAccessView,
                                                _In_ const FLOAT Values[4]) = 0;
    virtual void(ClearDepthStencilView)(_In_ ID3D11DepthStencilView * pDepthStencilView, _In_ UINT ClearFlags,
                                        _In_ FLOAT Depth, _In_ UINT8 Stencil) = 0;
    ...
    virtual void(ResolveSubresource)(_In_ ID3D11Resource * pDstResource, _In_ UINT DstSubresource,
    ...
    virtual void(HSSetShaderResources)(ID3D11ShaderResourceView *const *ppShaderResourceViews,
                                       _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT StartSlot,
                                       _In_ UINT PacketHeader) = 0;
    virtual void(HSSetShader)(_In_opt_ ID3D11HullShader * pHullShader) = 0;
    virtual void(HSSetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _In_reads_opt_(NumSamplers) ID3D11SamplerState *const *ppSamplers) = 0;
    virtual void(HSSetConstantBuffers)(
        ...
        _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot) UINT NumBuffers,
        _In_reads_opt_(NumBuffers) ID3D11Buffer *const *ppConstantBuffers) = 0;
    virtual void(DSSetShaderResources)(ID3D11ShaderResourceView *const *ppShaderResourceViews,
                                       _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT StartSlot,
                                       _In_ UINT PacketHeader) = 0;
    virtual void(DSSetShader)(_In_opt_ ID3D11DomainShader * pDomainShader) = 0;
    virtual void(DSSetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _In_reads_opt_(NumSamplers) ID3D11SamplerState *const *ppSamplers) = 0;
    virtual void(DSSetConstantBuffers)(
        ...
        _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot) UINT NumBuffers,
        _In_reads_opt_(NumBuffers) ID3D11Buffer *const *ppConstantBuffers) = 0;
    virtual void(CSSetShaderResources)(ID3D11ShaderResourceView *const *ppShaderResourceViews,
                                       _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT StartSlot,
                                       _In_ UINT PacketHeader) = 0;
    virtual void(CSSetUnorderedAccessViews)(_In_range_(0, D3D11_1_UAV_SLOT_COUNT - 1) UINT StartSlot,
                                            _In_range_(0, D3D11_1_UAV_SLOT_COUNT - StartSlot) UINT NumUAVs,
                                            _In_reads_opt_(NumUAVs)
                                                ID3D11UnorderedAccessView *const *ppUnorderedAccessViews,
                                            _In_reads_opt_(NumUAVs) const UINT *pUAVInitialCounts) = 0;
    virtual void(CSSetShader)(_In_opt_ ID3D11ComputeShader * pComputeShader) = 0;
    virtual void(CSSetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _In_reads_opt_(NumSamplers) ID3D11SamplerState *const *ppSamplers) = 0;
    ...
    virtual void(PSGetShader)(_Out_ ID3D11PixelShader * *ppPixelShader,
                              _Out_writes_opt_(*pNumClassInstances) ID3D11ClassInstance * *ppClassInstances,
                              _Inout_opt_ UINT * pNumClassInstances) = 0;
    virtual void(PSGetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _Out_writes_opt_(NumSamplers) ID3D11SamplerState * *ppSamplers) = 0;
    virtual void(VSGetShader)(_Out_ ID3D11VertexShader * *ppVertexShader,
                              _Out_writes_opt_(*pNumClassInstances) ID3D11ClassInstance * *ppClassInstances,
                              _Inout_opt_ UINT * pNumClassInstances) = 0;
    ...
    virtual void(IAGetVertexBuffers)(
        _In_range_(0, D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT - 1) UINT StartSlot,
        _In_range_(0, D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT - StartSlot) UINT NumBuffers,
        _Out_writes_opt_(NumBuffers) ID3D11Buffer * *ppVertexBuffers, _Out_writes_opt_(NumBuffers) UINT * pStrides,
        _Out_writes_opt_(NumBuffers) UINT * pOffsets) = 0;
    virtual void(IAGetIndexBuffer)(_Out_opt_ ID3D11Buffer * *pIndexBuffer, _Out_opt_ DXGI_FORMAT * Format,
                                   _Out_opt_ UINT * Offset) = 0;
    virtual void(GSGetConstantBuffers)(
        _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1) UINT StartSlot,
        _In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - StartSlot) UINT NumBuffers,
        _Out_writes_opt_(NumBuffers) ID3D11Buffer * *ppConstantBuffers) = 0;
    virtual void(GSGetShader)(_Out_ ID3D11GeometryShader * *ppGeometryShader,
                              _Out_writes_opt_(*pNumClassInstances) ID3D11ClassInstance * *ppClassInstances,
                              _Inout_opt_ UINT * pNumClassInstances) = 0;
    ...
    virtual void(VSGetShaderResources)(
        ...
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot) UINT NumViews,
        _Out_writes_opt_(NumViews) ID3D11ShaderResourceView * *ppShaderResourceViews) = 0;
    virtual void(VSGetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _Out_writes_opt_(NumSamplers) ID3D11SamplerState * *ppSamplers) = 0;
    ...
    virtual void(GSGetShaderResources)(
        ...
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot) UINT NumViews,
        _Out_writes_opt_(NumViews) ID3D11ShaderResourceView * *ppShaderResourceViews) = 0;
    virtual void(GSGetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _Out_writes_opt_(NumSamplers) ID3D11SamplerState * *ppSamplers) = 0;
    virtual void(OMGetRenderTargets)(_In_range_(0, D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT) UINT NumViews,
    ...
    virtual void(OMGetBlendState)(_Out_opt_ ID3D11BlendState * *ppBlendState,
                                  _Out_writes_all_opt_(4) FLOAT BlendFactor[4], _Out_opt_ UINT * pSampleMask) = 0;
    virtual void(OMGetDepthStencilState)(_Out_opt_ ID3D11DepthStencilState * *ppDepthStencilState,
                                         _Out_opt_ UINT * pStencilRef) = 0;
    virtual void(SOGetTargets)(_In_range_(0, D3D11_SO_BUFFER_SLOT_COUNT) UINT NumBuffers,
    ...
    virtual void(RSGetViewports)(_Inout_ /*_range(0, D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE)*/ UINT *
                                     pNumViewports,
    ...
    virtual void(HSGetShaderResources)(
        ...
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot) UINT NumViews,
        _Out_writes_opt_(NumViews) ID3D11ShaderResourceView * *ppShaderResourceViews) = 0;
    virtual void(HSGetShader)(_Out_ ID3D11HullShader * *ppHullShader,
                              _Out_writes_opt_(*pNumClassInstances) ID3D11ClassInstance * *ppClassInstances,
                              _Inout_opt_ UINT * pNumClassInstances) = 0;
    virtual void(HSGetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _Out_writes_opt_(NumSamplers) ID3D11SamplerState * *ppSamplers) = 0;
    ...
    virtual void(DSGetShaderResources)(
        ...
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot) UINT NumViews,
        _Out_writes_opt_(NumViews) ID3D11ShaderResourceView * *ppShaderResourceViews) = 0;
    virtual void(DSGetShader)(_Out_ ID3D11DomainShader * *ppDomainShader,
                              _Out_writes_opt_(*pNumClassInstances) ID3D11ClassInstance * *ppClassInstances,
                              _Inout_opt_ UINT * pNumClassInstances) = 0;
    virtual void(DSGetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _Out_writes_opt_(NumSamplers) ID3D11SamplerState * *ppSamplers) = 0;
    ...
    virtual void(CSGetShaderResources)(
        ...
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - StartSlot) UINT NumViews,
        _Out_writes_opt_(NumViews) ID3D11ShaderResourceView * *ppShaderResourceViews) = 0;
    virtual void(CSGetUnorderedAccessViews)(_In_range_(0, D3D11_PS_CS_UAV_REGISTER_COUNT - 1) UINT StartSlot,
    ...
    virtual void(CSGetShader)(_Out_ ID3D11ComputeShader * *ppComputeShader,
                              _Out_writes_opt_(*pNumClassInstances) ID3D11ClassInstance * *ppClassInstances,
                              _Inout_opt_ UINT * pNumClassInstances) = 0;
    virtual void(CSGetSamplers)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT StartSlot,
                                _In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - StartSlot) UINT NumSamplers,
                                _Out_writes_opt_(NumSamplers) ID3D11SamplerState * *ppSamplers) = 0;
    ...
    virtual HRESULT(FinishCommandList)(BOOL RestoreDeferredContextState,
...
{
  public:
    virtual void(CopySubresourceRegion1)(_In_ ID3D11Resource * pDstResource, _In_ UINT DstSubresource, _In_ UINT DstX,
                                         _In_ UINT DstY, _In_ UINT DstZ, _In_ ID3D11Resource * pSrcResource,
                                         _In_ UINT SrcSubresource, _In_opt_ const D3D11_BOX *pSrcBox,
                                         _In_ UINT CopyFlags) = 0;
    virtual void(UpdateSubresource1)(_In_ ID3D11Resource * pDstResource, _In_ UINT DstSubresource,
                                     _In_opt_ const D3D11_BOX *pDstBox, _In_ const void *pSrcData,
                                     _In_ UINT SrcRowPitch, _In_ UINT SrcDepthPitch, _In_ UINT CopyFlags) = 0;
    ...
    virtual void(VSGetConstantBuffers1)(
        ...
        _Out_writes_opt_(NumBuffers) UINT * pFirstConstant, _Out_writes_opt_(NumBuffers) UINT * pNumConstants) = 0;
    ...
    virtual void(CSGetConstantBuffers1)(
        ...
        _Out_writes_opt_(NumBuffers) ID3D11Buffer * *ppConstantBuffers,
        _Out_writes_opt_(NumBuffers) UINT * pFirstConstant, _Out_writes_opt_(NumBuffers) UINT * pNumConstants) = 0;
    virtual void(SwapDeviceContextState)(_In_ ID3DDeviceContextState * pState,
                                         _Out_opt_ ID3DDeviceContextState * *ppPreviousState) = 0;
    virtual void(ClearView)(_In_ ID3D11View * pView, _In_ const FLOAT Color[4],
                            _In_reads_opt_(NumRects) const D3D11_RECT *pRect, UINT NumRects) = 0;
    virtual void(DiscardView1)(_In_ ID3D11View * pResourceView, _In_reads_opt_(NumRects) const D3D11_RECT *pRects,
                               UINT NumRects) = 0;
};
...
{
  public:
    virtual HRESULT(UpdateTileMappings)(
        _In_ ID3D11Resource * pTiledResource, _In_ UINT NumTiledResourceRegions,
        ...
        _In_opt_ ID3D11Buffer *pTilePool, _In_ UINT NumRanges, _In_reads_opt_(NumRanges) const UINT *pRangeFlags,
        _In_reads_opt_(NumRanges) const UINT *pTilePoolStartOffsets,
        _In_reads_opt_(NumRanges) const UINT *pRangeTileCounts, _In_ UINT Flags) = 0;
    virtual HRESULT(CopyTileMappings)(_In_ ID3D11Resource * pDestTiledResource,
                                      ...
                                      _In_ const D3D11_TILE_REGION_SIZE *pTileRegionSize, _In_ UINT Flags) = 0;
    virtual void(CopyTiles)(_In_ ID3D11Resource * pTiledResource,
                            _In_ const D3D11_TILED_RESOURCE_COORDINATE *pTileRegionStartCoordinate,
                            _In_ const D3D11_TILE_REGION_SIZE *pTileRegionSize, _In_ ID3D11Buffer *pBuffer,
                            _In_ UINT64 BufferStartOffsetInBytes, _In_ UINT Flags) = 0;
    virtual void(UpdateTiles)(_In_ ID3D11Resource * pDestTiledResource,
                              _In_ const D3D11_TILED_RESOURCE_COORDINATE *pDestTileRegionStartCoordinate,
                              _In_ const D3D11_TILE_REGION_SIZE *pDestTileRegionSize, _In_ const void *pSourceTileData,
                              _In_ UINT Flags) = 0;
    virtual HRESULT(ResizeTilePool)(_In_ ID3D11Buffer * pTilePool, _In_ UINT64 NewSizeInBytes) = 0;
    virtual void(TiledResourceBarrier)(_In_opt_ ID3D11DeviceChild * pTiledResourceOrViewAccessBeforeBarrier,
...
D3DINTERFACE(ID3D11DeviceContextX, 48800095, 7134, 4be7, 91, 86, b8, 6b, ec, b2, 64, 77) : public ID3D11DeviceContext2
{
  public:
    ...
    virtual HRESULT(GetCounterData)(_In_ ID3D11CounterSampleX * pCounterSample, _Out_ D3D11X_COUNTER_DATA * pData,
                                    _In_ UINT GetCounterDataFlags) = 0;
    ...
    virtual void(RemapConstantBufferInheritance)(_In_ D3D11_STAGE Stage, _In_ UINT Slot, _In_ D3D11_STAGE InheritStage,
                                                 _In_ UINT InheritSlot) = 0;
    virtual void(RemapShaderResourceInheritance)(_In_ D3D11_STAGE Stage, _In_ UINT Slot, _In_ D3D11_STAGE InheritStage,
                                                 _In_ UINT InheritSlot) = 0;
    virtual void(RemapSamplerInheritance)(_In_ D3D11_STAGE Stage, _In_ UINT Slot, _In_ D3D11_STAGE InheritStage,
                                          _In_ UINT InheritSlot) = 0;
    virtual void(RemapVertexBufferInheritance)(_In_ UINT Slot, _In_ UINT InheritSlot) = 0;
    virtual void(PSSetFastConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                              UINT Slot,
                                          _In_ ID3D11Buffer * pConstantBuffer) = 0;
    virtual void(PSSetFastShaderResource)(_In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
                                          _In_ ID3D11ShaderResourceView * pShaderResourceView) = 0;
    virtual void(PSSetFastSampler)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT Slot,
                                   _In_ ID3D11SamplerState * pSampler) = 0;
    virtual void(VSSetFastConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                              UINT Slot,
                                          _In_ ID3D11Buffer * pConstantBuffer) = 0;
    virtual void(VSSetFastShaderResource)(_In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
                                          _In_ ID3D11ShaderResourceView * pShaderResourceView) = 0;
    virtual void(VSSetFastSampler)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT Slot,
                                   _In_ ID3D11SamplerState * pSampler) = 0;
    virtual void(GSSetFastConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                              UINT Slot,
                                          _In_ ID3D11Buffer * pConstantBuffer) = 0;
    virtual void(GSSetFastShaderResource)(_In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
                                          _In_ ID3D11ShaderResourceView * pShaderResourceView) = 0;
    virtual void(GSSetFastSampler)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT Slot,
                                   _In_ ID3D11SamplerState * pSampler) = 0;
    virtual void(CSSetFastConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                              UINT Slot,
                                          _In_ ID3D11Buffer * pConstantBuffer) = 0;
    virtual void(CSSetFastShaderResource)(_In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
                                          _In_ ID3D11ShaderResourceView * pShaderResourceView) = 0;
    virtual void(CSSetFastSampler)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT Slot,
                                   _In_ ID3D11SamplerState * pSampler) = 0;
    virtual void(HSSetFastConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                              UINT Slot,
                                          _In_ ID3D11Buffer * pConstantBuffer) = 0;
    virtual void(HSSetFastShaderResource)(_In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
                                          _In_ ID3D11ShaderResourceView * pShaderResourceView) = 0;
    virtual void(HSSetFastSampler)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT Slot,
                                   _In_ ID3D11SamplerState * pSampler) = 0;
    virtual void(DSSetFastConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                              UINT Slot,
                                          _In_ ID3D11Buffer * pConstantBuffer) = 0;
    virtual void(DSSetFastShaderResource)(_In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
                                          _In_ ID3D11ShaderResourceView * pShaderResourceView) = 0;
    virtual void(DSSetFastSampler)(_In_range_(0, D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - 1) UINT Slot,
                                   _In_ ID3D11SamplerState * pSampler) = 0;
    virtual void(IASetFastVertexBuffer)(_In_range_(0, D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
                                        _In_ ID3D11Buffer * pVertexBuffer, _In_ UINT Stride) = 0;
    virtual void(IASetFastIndexBuffer)(_In_ UINT HardwareIndexFormat, _In_ ID3D11Buffer * pIndexBuffer) = 0;
    virtual void(PSSetPlacementConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                                   UINT Slot,
                                               _In_ ID3D11Buffer * pConstantBuffer, _In_ void *pBaseAddress) = 0;
    virtual void(PSSetPlacementShaderResource)(
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
        _In_ ID3D11ShaderResourceView * pShaderResourceView, _In_ void *pBaseAddress) = 0;
    virtual void(VSSetPlacementConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                                   UINT Slot,
                                               _In_ ID3D11Buffer * pConstantBuffer, _In_ void *pBaseAddress) = 0;
    virtual void(VSSetPlacementShaderResource)(
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
        _In_ ID3D11ShaderResourceView * pShaderResourceView, _In_ void *pBaseAddress) = 0;
    virtual void(GSSetPlacementConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                                   UINT Slot,
                                               _In_ ID3D11Buffer * pConstantBuffer, _In_ void *pBaseAddress) = 0;
    virtual void(GSSetPlacementShaderResource)(
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
        _In_ ID3D11ShaderResourceView * pShaderResourceView, _In_ void *pBaseAddress) = 0;
    virtual void(CSSetPlacementConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                                   UINT Slot,
                                               _In_ ID3D11Buffer * pConstantBuffer, _In_ void *pBaseAddress) = 0;
    virtual void(CSSetPlacementShaderResource)(
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
        _In_ ID3D11ShaderResourceView * pShaderResourceView, _In_ void *pBaseAddress) = 0;
    virtual void(HSSetPlacementConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                                   UINT Slot,
                                               _In_ ID3D11Buffer * pConstantBuffer, _In_ void *pBaseAddress) = 0;
    virtual void(HSSetPlacementShaderResource)(
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
        _In_ ID3D11ShaderResourceView * pShaderResourceView, _In_ void *pBaseAddress) = 0;
    virtual void(DSSetPlacementConstantBuffer)(_In_range_(0, D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1)
                                                   UINT Slot,
                                               _In_ ID3D11Buffer * pConstantBuffer, _In_ void *pBaseAddress) = 0;
    virtual void(DSSetPlacementShaderResource)(
        _In_range_(0, D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
        _In_ ID3D11ShaderResourceView * pShaderResourceView, _In_ void *pBaseAddress) = 0;
    virtual void(IASetPlacementVertexBuffer)(_In_range_(0, D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT - 1) UINT Slot,
                                             _In_ ID3D11Buffer * pVertexBuffer, _In_ void *pBaseAddress,
                                             _In_ UINT Stride) = 0;
    virtual void(IASetPlacementIndexBuffer)(_In_ UINT HardwareIndexFormat, _In_ ID3D11Buffer * pIndexBuffer,
                                            _In_ void *pBaseAddress) = 0;
    virtual void(HSSetTessellationParameters)(
        _In_opt_ const D3D11X_TESSELLATION_PARAMETERS *pTessellationParameters) = 0;
    virtual void(HSGetLastUsedTessellationParameters)(_Inout_ D3D11X_TESSELLATION_PARAMETERS *
                                                      pTessellationParameters) = 0;
    ...
    virtual void(CSPlaceUnorderedAccessView)(_In_range_(0, D3D11_1_UAV_SLOT_COUNT - 1) UINT Slot,
    ...
    virtual HRESULT(StartThreadTrace)(_In_ const D3D11X_THREAD_TRACE_DESC *pDesc, _In_ void *pDstAddressShaderEngine0,
                                      _In_ void *pDstAddressShaderEngine1, _In_ SIZE_T BufferSizeBytes) = 0;
    ...
    virtual void(ClearRenderTargetViewX)(_In_ ID3D11RenderTargetView * pRenderTargetView, _In_ UINT Flags,
                                         _In_ const FLOAT ColorRGBA[4]) = 0;
    ...
    virtual void(DecompressResource)(_In_ ID3D11Resource * pDstResource, _In_ UINT DstSubresource,
                                     _In_opt_ const D3D11X_POINT *pDstPoint, _In_ ID3D11Resource *pSrcResource,
                                     _In_ UINT SrcSubresource, _In_opt_ const D3D11X_RECT *pSrcRect,
                                     _In_ DXGI_FORMAT DecompressFormat, _In_ UINT DecompressFlags) = 0;
    virtual void(DecompressResourceX)(
        _In_ D3D11X_DESCRIPTOR_RESOURCE * pDstResource, _In_ UINT DstSubresource,
        _In_opt_ const D3D11X_POINT *pDstPoint, _In_ D3D11X_DESCRIPTOR_RESOURCE *pSrcResource, _In_ UINT SrcSubresource,
        _In_opt_ const D3D11X_RECT *pSrcRect, _In_ D3D11X_FORMAT DecompressFormat, _In_ UINT DecompressFlags) = 0;
    ...
    virtual void(MultiDrawIndexedInstancedIndirect)(_In_ UINT PrimitiveCount, _Inout_ ID3D11Buffer * pBufferForArgs,
                                                    _In_ UINT AlignedByteOffsetForArgs,
                                                    _In_ UINT StrideByteOffsetForArgs, _In_ UINT Flags) = 0;
    virtual void(MultiDrawInstancedIndirect)(_In_ UINT PrimitiveCount, _Inout_ ID3D11Buffer * pBufferForArgs,
                                             _In_ UINT AlignedByteOffsetForArgs, _In_ UINT StrideByteOffsetForArgs,
                                             _In_ UINT Flags) = 0;
    virtual void(MultiDrawIndexedInstancedIndirectAuto)(
        _Inout_ ID3D11Buffer * pBufferForPrimitiveCount, _In_ UINT AlignedByteOffsetForPrimitiveCount,
        _Inout_ ID3D11Buffer * pBufferForArgs, _In_ UINT AlignedByteOffsetForArgs, _In_ UINT StrideByteOffsetForArgs,
        _In_ UINT Flags) = 0;
    virtual void(MultiDrawInstancedIndirectAuto)(
        _Inout_ ID3D11Buffer * pBufferForPrimitiveCount, _In_ UINT AlignedByteOffsetForPrimitiveCount,
        _Inout_ ID3D11Buffer * pBufferForArgs, _In_ UINT AlignedByteOffsetForArgs, _In_ UINT StrideByteOffsetForArgs,
        _In_ UINT Flags) = 0;
    virtual HRESULT(RSGetMSAASettingsForQuality)(_Inout_opt_ D3D11X_MSAA_SCAN_CONVERTER_SETTINGS * pMSAASCSettings,
                                                 ...
                                                 _In_ UINT LogSampleCount, _In_ UINT SampleQuality) = 0;
    ...
    virtual void(RSSetSamplePositions)(_In_opt_ const D3D11X_MSAA_SAMPLE_PRIORITIES *pSamplesPriorities,
                                       _In_opt_ const D3D11X_MSAA_SAMPLE_POSITIONS *pSamplePositions) = 0;
    ...
    virtual void(WriteGDS)(_In_ D3D11X_GDS_REGION_TYPE RegionType, _In_ UINT OffsetDwords, _In_ UINT NumDwords,
                           _In_ const UINT *pCounterValues, _In_ UINT Flags) = 0;
    virtual void(ReadGDS)(_In_ D3D11X_GDS_REGION_TYPE RegionType, _In_ UINT OffsetDwords, _In_ UINT NumDwords,
                          _Inout_ UINT * pCounterValues, _In_ UINT Flags) = 0;
    virtual void(VSSetShaderUserData)(_In_ UINT StartSlot, _In_ UINT NumRegisters,
                                      _In_reads_(NumRegisters) const UINT *pData) = 0;
    virtual void(HSSetShaderUserData)(_In_ UINT StartSlot, _In_ UINT NumRegisters,
                                      _In_reads_(NumRegisters) const UINT *pData) = 0;
    virtual void(DSSetShaderUserData)(_In_ UINT StartSlot, _In_ UINT NumRegisters,
                                      _In_reads_(NumRegisters) const UINT *pData) = 0;
    virtual void(GSSetShaderUserData)(_In_ UINT StartSlot, _In_ UINT NumRegisters,
                                      _In_reads_(NumRegisters) const UINT *pData) = 0;
    virtual void(PSSetShaderUserData)(_In_ UINT StartSlot, _In_ UINT NumRegisters,
                                      _In_reads_(NumRegisters) const UINT *pData) = 0;
    virtual void(CSSetShaderUserData)(_In_ UINT StartSlot, _In_ UINT NumRegisters,
                                      _In_reads_(NumRegisters) const UINT *pData) = 0;
    virtual void(InsertWaitOnMemory)(_In_ const void *pAddress, _In_ UINT Flags,
                                     _In_ D3D11_COMPARISON_FUNC ComparisonFunction, _In_ UINT ReferenceValue,
                                     _In_ UINT Mask) = 0;
    ...
    virtual void(StoreConstantRam)(_In_ UINT Flags, _In_ ID3D11Buffer * pBuffer, _In_ UINT BufferOffsetInBytes,
                                   _In_ UINT CeRamOffsetInBytes, _In_ UINT SizeInBytes) = 0;
    virtual void(LoadConstantRam)(_In_ UINT Flags, _In_ ID3D11Buffer * pBuffer, _In_ UINT BufferOffsetInBytes,
                                  _In_ UINT CeRamOffsetInBytes, _In_ UINT SizeInBytes) = 0;
    virtual void(WriteQuery)(_In_ D3D11_QUERY QueryType, _In_ UINT QueryIndex, _In_ UINT Flags,
                             _In_ ID3D11Buffer * pBuffer, _In_ UINT OffsetInBytes, _In_ UINT StrideInBytes) = 0;
    virtual void(ResetQuery)(_In_ D3D11_QUERY QueryType, _In_ UINT QueryIndex, _In_ UINT Flags) = 0;
    virtual void(ConfigureQuery)(_In_ D3D11_QUERY QueryType, _In_ const void *pConfiguration,
                                 _In_ UINT ConfigurationSize) = 0;
    virtual void(SetShaderUserData)(_In_ D3D11X_HW_STAGE ShaderStage, _In_ UINT StartSlot, _In_ UINT NumRegisters,
                                    _In_reads_(NumRegisters) const UINT *pData) = 0;
    virtual void(SetPixelShaderDepthForceZOrder)(_In_ BOOL ForceOrder) = 0;
    virtual void(SetPredicationFromQuery)(_In_ D3D11_QUERY QueryType, _In_ ID3D11Buffer * pBuffer,
                                          _In_ UINT OffsetInBytes, _In_ UINT Flags) = 0;
    ...
    virtual void(InsertWaitOnMemory64)(_In_ const void *pAddress, _In_ UINT Flags,
                                       _In_ D3D11_COMPARISON_FUNC ComparisonFunction, _In_ UINT64 ReferenceValue) = 0;
    virtual void(LoadConstantRamImmediate)(_In_ UINT Flags, _In_ const void *pBuffer, _In_ UINT CeRamOffsetInBytes,
                                           _In_ UINT SizeInBytes) = 0;
    virtual void(SetScreenExtentsQuery)(_In_ UINT Value) = 0;
    virtual void(CollectScreenExtents)(_In_ UINT Flags, _In_ UINT AddressCount,
                                       _In_reads_(AddressCount) const UINT64 *pDestinationAddresses, _In_ USHORT ZMin,
                                       _In_ USHORT ZMax) = 0;
...
class device_context_x : public wdi::ID3D11DeviceContextX
{
  ...
  public:
    ...
    void IASetVertexBuffers(UINT StartSlot, UINT NumBuffers, ID3D11Buffer *const *ppVertexBuffers, const UINT *pStrides,
    ...
    void ClearUnorderedAccessViewUint(ID3D11UnorderedAccessView *pUnorderedAccessView, const UINT Values[4]) override;
    void ClearUnorderedAccessViewFloat(ID3D11UnorderedAccessView *pUnorderedAccessView, const FLOAT Values[4]) override;
    ...
    void PSGetShaderResources(UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView **ppShaderResourceViews) override;
    ...
    void VSGetShaderResources(UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView **ppShaderResourceViews) override;
    ...
    void GSGetShaderResources(UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView **ppShaderResourceViews) override;
    ...
    void HSGetShaderResources(UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView **ppShaderResourceViews) override;
    ...
    void DSGetShaderResources(UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView **ppShaderResourceViews) override;
    ...
    void CSGetShaderResources(UINT StartSlot, UINT NumViews, ID3D11ShaderResourceView **ppShaderResourceViews) override;
    ...
    void CSSetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer *const *ppConstantBuffers,
                               const UINT *pFirstConstant, const UINT *pNumConstants) override;
    void VSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer **ppConstantBuffers, UINT *pFirstConstant,
                               UINT *pNumConstants) override;
    void HSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer **ppConstantBuffers, UINT *pFirstConstant,
                               UINT *pNumConstants) override;
    void DSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer **ppConstantBuffers, UINT *pFirstConstant,
                               UINT *pNumConstants) override;
    void GSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer **ppConstantBuffers, UINT *pFirstConstant,
                               UINT *pNumConstants) override;
    void PSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer **ppConstantBuffers, UINT *pFirstConstant,
                               UINT *pNumConstants) override;
    void CSGetConstantBuffers1(UINT StartSlot, UINT NumBuffers, ID3D11Buffer **ppConstantBuffers, UINT *pFirstConstant,
                               UINT *pNumConstants) override;
    ...
    HRESULT UpdateTileMappings(ID3D11Resource *pTiledResource, UINT NumTiledResourceRegions,
                               ...
                               const UINT *pRangeTileCounts, UINT Flags) override;
    ...
    void CopyTiles(ID3D11Resource *pTiledResource, const D3D11_TILED_RESOURCE_COORDINATE *pTileRegionStartCoordinate,
    ...
    void DSSetPlacementShaderResource(UINT Slot, ID3D11ShaderResourceView *pShaderResourceView,
                                      void *pBaseAddress) override;
    void IASetPlacementVertexBuffer(UINT Slot, ID3D11Buffer *pVertexBuffer, void *pBaseAddress, UINT Stride) override;
    void IASetPlacementIndexBuffer(UINT HardwareIndexFormat, ID3D11Buffer *pIndexBuffer, void *pBaseAddress) override;
    ...
    void DecompressResourceX(wdi::D3D11X_DESCRIPTOR_RESOURCE *pDstResource, UINT DstSubresource,
                             const wdi::D3D11X_POINT *pDstPoint, wdi::D3D11X_DESCRIPTOR_RESOURCE *pSrcResource,
                             UINT SrcSubresource, const wdi::D3D11X_RECT *pSrcRect, wdi::D3D11X_FORMAT DecompressFormat,
                             UINT DecompressFlags) override;
    ...
    void MultiDrawIndexedInstancedIndirect(UINT PrimitiveCount, ID3D11Buffer *pBufferForArgs,
                                           ...
                                           UINT Flags) override;
    void MultiDrawInstancedIndirect(UINT PrimitiveCount, ID3D11Buffer *pBufferForArgs, UINT AlignedByteOffsetForArgs,
    ...
    void MultiDrawIndexedInstancedIndirectAuto(ID3D11Buffer *pBufferForPrimitiveCount,
                                               UINT AlignedByteOffsetForPrimitiveCount, ID3D11Buffer *pBufferForArgs,
                                               UINT AlignedByteOffsetForArgs, UINT StrideByteOffsetForArgs,
                                               UINT Flags) override;
    void MultiDrawInstancedIndirectAuto(ID3D11Buffer *pBufferForPrimitiveCount, UINT AlignedByteOffsetForPrimitiveCount,
    ...
    HRESULT RSGetMSAASettingsForQuality(wdi::D3D11X_MSAA_SCAN_CONVERTER_SETTINGS *pMSAASCSettings,
                                        wdi::D3D11X_MSAA_EQAA_SETTINGS *pEQAASettings,
                                        wdi::D3D11X_MSAA_SAMPLE_PRIORITIES *pCentroidPriorities,
                                        wdi::D3D11X_MSAA_SAMPLE_POSITIONS *pSamplePositions, UINT LogSampleCount,
                                        UINT SampleQuality) override;
    ...
    void WriteGDS(wdi::D3D11X_GDS_REGION_TYPE RegionType, UINT OffsetDwords, UINT NumDwords, const UINT *pCounterValues,
                  UINT Flags) override;
    ...
    void SetPredicationFromQuery(D3D11_QUERY QueryType, ID3D11Buffer *pBuffer, UINT OffsetInBytes, UINT Flags) override;
    ...
    void LoadConstantRamImmediate(UINT Flags, const void *pBuffer, UINT CeRamOffsetInBytes, UINT SizeInBytes) override;
